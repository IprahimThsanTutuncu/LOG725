...
#include "../../../../../game/src/gameObjects/NPC/OrangeCuteRenderComponent.h"
...
void EditorState::load_scene(std::string path)
{
    ...
    scene = new Scene(this, path, VIEW_TYPE::DYNAMIC_VIEW);
    ...
    VoxelTerrainData* voxelTerrainData = scene->get_child("terrain")->getData<VoxelTerrainData>(DATA_TYPE::VOXEL_TERRAIN);
...
void EditorState::gameObjectAssetEdit()
{
    ...
    if (multiSelectionActivated && multipleSelectedGameObject.size() > 0) {
        ...
        for (auto go : multipleSelectedGameObject) {
            ...
            if (go->hasData(DATA_TYPE::LIGHT)) {
                LightData* ld = go->getData<LightData>(DATA_TYPE::LIGHT);
        ...
        if (selectedGameObject->hasData(DATA_TYPE::LIGHT)) {
            LightData* ld = selectedGameObject->getData<LightData>(DATA_TYPE::LIGHT);
            ...
            Transform* transform = selectedGameObject->getData<Transform>(DATA_TYPE::TRANSFORM);
        ...
        if (selectedGameObject->hasData(DATA_TYPE::EDITABLE_IMAGE)) {
            EditableImageData* editableImageData = selectedGameObject->getData<EditableImageData>(DATA_TYPE::EDITABLE_IMAGE);
        ...
        if (selectedGameObject->hasData(DATA_TYPE::EDITABLE_SPRITE_ANIMATION)) {
            EditableSpriteAnimationData* e = selectedGameObject->getData<EditableSpriteAnimationData>(DATA_TYPE::EDITABLE_SPRITE_ANIMATION);
