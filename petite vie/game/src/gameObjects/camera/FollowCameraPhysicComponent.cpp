...
void FollowCameraPhysicComponent::init(Scene& scene)
{
	...
	if (!parent->hasData(DATA_TYPE::CAMERA))
		parent->setData(DATA_TYPE::CAMERA, new CameraData());
...
{
	CameraData* cd = parent->getData<CameraData>(DATA_TYPE::CAMERA);
	VoxelTerrainData* vt = scene.get_child("terrain")->getData<VoxelTerrainData>(DATA_TYPE::VOXEL_TERRAIN);
	Transform* player_transform = scene.get_child("player")->getData<Transform>(DATA_TYPE::TRANSFORM);
	...
	if (vt) {
		...
		*/
		...
		if (target->hasData(DATA_TYPE::PLAYER)) {
			...
			player_position = target->getData<Transform>(DATA_TYPE::TRANSFORM)->position;
		}else
			player_data = nullptr;
