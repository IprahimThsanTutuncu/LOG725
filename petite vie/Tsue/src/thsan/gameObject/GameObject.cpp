    ...
    m_renderComponent(renderComp)
{
    ...
    if(!hasData(DATA_TYPE::TRANSFORM))
        setData(DATA_TYPE::TRANSFORM, new Transform());

    transform = getData<Transform>(DATA_TYPE::TRANSFORM);
...
GameObject::GameObject(const GameObject& other)
{
    ...
    *m_renderComponent = *other.m_renderComponent;
    ...
    m_DataTypeHolder = other.m_DataTypeHolder;
}
...
bool GameObject::hasData(DATA_TYPE type)
{
    return m_DataTypeHolder.hasData(type);
}

void GameObject::setData(DATA_TYPE target, DataType* comp)
{
    m_DataTypeHolder.setData(target, *comp);
}
...
{
    if (!hasChild(go)) {
        Transform* child_transform = go->getData<Transform>(DATA_TYPE::TRANSFORM);
        child_transform->parent = this->getData<Transform>(DATA_TYPE::TRANSFORM);
...
void GameObject::removeChild(const std::string& name)
{
    ...
    if (child_location != m_childrenMap.end()) {
        ...
        Transform* child_transform = child->getData<Transform>(DATA_TYPE::TRANSFORM);
